...
**Transformer**：
选用正余弦函数：在[-1,1]之间，通过调整移动频率因子，将所有的position赋予独特的高维表达。
...
Step1:从dataset中取出一些sample-prompts->让人类写出他们期待的responses->用这个(X,Y)对GPT进行fine-tuning

Step2:再从dataset中取出一些sample-prompts和针对同一prompt模型给出的几个不同responses->让人类给这几个responses进行评分->把这些prompts和带评分的responses一起训练形成一个reward model

Step3:继续从dataset中取出一些prompts，让模型生成response->根据这个response，reward model给出评分->用这个评分通过PPO去更新模型参数
