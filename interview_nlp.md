...
1. 使用Zero组件，来优化训练过程中的内存问题。通过三个方面，分别优化梯度储存、优化器状态储存、模型参数。将这些数据都分片储存在不同的设备上，而不是在每个设备上储存全量数据，只在必要时重构<br>
2. 混合精度训练<br>
3. 梯度累计，如果显存不足，需要用小批次训练，可以进行梯度累积，累积到一定梯度再进行更新参数<br>
4. 数据并行与模型并行：每个 GPU 处理一部分输入数据，并独立计算损失和梯度，之后通过通信机制（如 all-reduce）将每个 GPU 上的梯度汇总。DeepSpeed 支持模型并行，通过将模型的不同层分配到多个 GPU 上进行计算。在这种情况下，不同 GPU 之间进行流水线操作，以提高计算效率并分摊显存压力


#### 19. VLLM

用于高效部署和推理大型语言模型的开源工具，旨在提升大型语言模型（LLM）在推理阶段的吞吐量和资源利用效率。VLLM通过引入一种名为**PagedAttention**的新型注意力机制，实现了高效的内存管理和并行计算，支持高吞吐量的多请求并行推理。

PagedAttention的工作机制<br>
1. 按需加载： 当模型进行注意力计算时，它不需要把所有的键和值一次性存放在显存里。相反，它会根据当前的计算需求，只加载当前步骤需要的部分页面，从而节省显存空间。<br>
2. 共享： 如果多个用户请求中的某些部分使用了相同的键和值（即相同的上下文或文本片段），这些请求可以共享相同的内存页面。这样，系统不用为每个请求重复存储相同的键和值，大大节省了内存资源。

**对比**<br>
在传统的注意力机制中，所有的键和值都需要存储在显存中，尤其在处理长序列或多个并行请求时，显存占用非常高。
而PagedAttention通过分页和按需加载的方式减少了显存的使用，并且通过共享机制提高了内存的利用率。这让系统能够处理更多的并行请求，而不会因为显存耗尽而被限制
...
Momentum-> **向量 \(v_t\)** 可以看作是一个**累加的历史信息**，在每一步迭代中，当前的速度向量 \(v_t\) 是由**上一轮的速度向量**乘以一个动量系数（通常是 \(\beta\)）和**当前梯度信息**乘以另一个系数（通常是 \(1 - \beta\)）共同决定的。

当使用 AdamW 时，正则化项λθ是在**参数更新时直接加入**的，与梯度更新是独立的；而在 Adam 中，正则化项**被加入到损失函数的梯度计算**中，因此正则化的效果受到梯度动量和自适应学习率的影响
...
#### 16. Python需不需要编译，跟c++有什么区别

Python是**解释型语言**，Python代码在执行时由Python解释器逐行翻译成机器代码。这意味着代码的每一行在运行时都会被逐步解释、执行。所以Python代码不需要显式编译为机器代码。

C++ 是编译型语言。C++ 代码在执行之前需要通过编译器（如 g++ 或 clang++）将源码编译为机器码（通常是二进制可执行文件）


#### 17. 特征值和特征向量

对于一个矩阵A和一个非零向量v，如果存在一个标量λ使得满足下式：A⋅v=λ⋅v 那么v就是矩阵 A的特征向量。λ就是对应的特征值。

特征向量是经过矩阵A作用后，方向不变的向量。尽管矩阵A会对向量进行变换，但它不会改变特征向量的方向，只会拉伸或压缩该向量。
特征值是特征向量被拉伸或压缩的倍数，即矩阵作用于特征向量时的“尺度因子”。<br>
